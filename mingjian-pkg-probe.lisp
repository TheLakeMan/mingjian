;;; mingjian-pkg-probe.lisp — proves mingjian is a valid, cwd-independent package.
;;; Copyright (c) 2026 Nicholas Vermeulen
;;; SPDX-License-Identifier: AGPL-3.0-or-later
;;;
;;; run_tests.sh copies mingjian into a throwaway $HOME/.rusty/packages/mingjian
...
;;;   PKG-ENTRY-OK       — loading the manifest's `main` brings the library up
;;;                        despite cwd-relative `load`
;;;   SELFCHECK-GUARDED  — mj-self-check degrades (no crash) without pkg.lisp

(define pkgdir (string-append (shell "printf $HOME") "/.rusty/packages/mingjian"))
...
(println
  (if (and (equal? "mingjian" (m-get 'name))
           (string? (m-get 'version))
           (equal? "mingjian-pkg.lisp" (m-get 'main)))
    "MANIFEST-OK" "MANIFEST-FAIL"))

;; (2) The package entry loads the audit library from this foreign cwd.
(load (string-append pkgdir "/mingjian-pkg.lisp"))
(define fn-type (type-of (lambda (x) x)))
(println
  (try-catch
    (if (and (equal? fn-type (type-of mj-verify-run))   ; mingjian.lisp loaded
             (equal? fn-type (type-of mj-anchor)))        ; mingjian.lisp loaded
...
(println
  (try-catch
    (if (member (car (mj-self-check)) '(pkg-not-loaded no-lock))
